/**
 * STAGING — тестовая копия seasons-config.js (для проверки на тестовом домене).
 * Боевой сайт использует seasons-config.js без суффикса -staging.
 *
 * При смене сезона: поменять activeSeasonId и блок в seasons[].
 * Чтобы проверить скрытие вкладки «Сезон»: временно очистите match.gids и titleContains.
 */
