/**
 * STAGING — тестовая версия games-hub (не для боевого сайта).
 * Отличие: если сезонных игр нет — скрываются вкладка «Сезон» и блок календаря сезона.
 * Требует: jQuery, seasons-config-staging.js, #kvGamesHub
 */
(function () {
  ...
  function initKvGamesHub($) {
    ...
    if (hasHub && games.length) {
      ...
      $("#kv-empty-season").hide();
    }

    var TAB_MAP = ["live", "season", "upcoming", "real", "virtual", "all"];

    /** Нет сезонных игр — убираем вкладку, панель и таблицу календаря сезона */
    function applySeasonVisibility() {
      var seasonN = count("season");
      var show = seasonN > 0;
      $("html").toggleClass("kv-no-season-games", !show);
      $('.kv-tabs button[data-tab="season"]').css("display", show ? "" : "none");
      $("#kv-panel-season").css("display", show ? "" : "none");
      $(".kv-season-block").css("display", show ? "" : "none");
      $("#kvHero").css("display", show ? "" : "none");
      if (!show) {
        $("#divCalendar2").empty();
        $("#kv-empty-season").hide();
      }
    ...
    function showTab(key) {
      if (key === "season" && $("html").hasClass("kv-no-season-games")) {
        key = count("live") > 0 ? "live" : "upcoming";
      }
      var panelId = "kv-panel-" + key;
      var $btn = $('.kv-tabs button[data-tab="' + key + '"]');
      if (!$btn.length || $btn.css("display") === "none") {
        key = count("live") > 0 ? "live" : "upcoming";
        panelId = "kv-panel-" + key;
        $btn = $('.kv-tabs button[data-tab="' + key + '"]');
      }
